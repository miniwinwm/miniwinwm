...
static uint8_t  is_init[USB_NUM_USBIP] = {USB_NO};
...
#if (BSP_CFG_RTOS_USED == 1)
    os_err = usb_cstd_get_semaphore(ctrl);
    if (USB_SUCCESS != os_err)
    {
        return USB_ERR_NG;
    }
#endif /* (BSP_CFG_RTOS_USED == 1) */
...
#if (BSP_CFG_RTOS_USED == 1)
    os_err = usb_cstd_get_semaphore(p_ctrl);
    if (USB_SUCCESS != os_err)
    {
        return USB_ERR_NG;
    }
...
/* DEL START 2018.10.09 BBU SDD SPD SWD */
/*  Avoid warning"usb_err_t   os_err"*/
//#if !defined(USB_CFG_HVND_USE) && !defined(USB_CFG_PVND_USE)
//    return USB_ERR_NG;
//#else
/* DEL END   2018.10.09 BBU SDD SPD SWD */
    usb_er_t    err;
    #if defined(USB_CFG_HVND_USE)
        usb_info_t  info;
    #endif /* defined(USB_CFG_HVND_USE) */
