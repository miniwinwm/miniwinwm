...
/**
  * @brief  Returns if the touch screen is touched or not.
  * @param  None
  * @retval true for touched or false for not touched
  */
bool IOE_TP_GetTouchState(void)
{
  /* Check if the Touch detect event happened */
  if ((I2C_ReadDeviceRegister(IOE_REG_TP_CTRL) & 0x80) == 0x80)
  {
	  return true;
  }

  /* clear fifo & re-enable it again */
...
/**
  * @brief  Returns if the FIFO is empty or not.
  * @param  None
  * @retval true or false.
  */
bool IOE_TP_IsFifoEmpty(void)
{
	uint8_t tmp = I2C_ReadDeviceRegister(IOE_REG_FIFO_STA);
...
TP_STATE* IOE_TP_GetState(void)
{
  ...
  {
	  TP_State.X = IOE_TP_Read_X();
	  TP_State.Y = IOE_TP_Read_Y();
  }
...
//  I2C_WriteDeviceRegister(IOE_REG_TP_FRACT_XYZ, 0x01);
  ...
  /* Use no tracking index, touch-panel controller operation mode (XY) and
     enable the TSC */
  ...
  TP_State.TouchDetected = TP_State.X = TP_State.Y = 0;
